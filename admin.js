...
const K = { apps: 'ekuveri:apps', site: 'ekuveri:site', docs: 'ekuveri:docs', images: 'ekuveri:images' };
...
function sanitizeImages(arr) {
  if (!Array.isArray(arr)) return [];
  return arr.slice(0, 60).map((m) => ({
    id: m && m.id ? String(m.id).slice(0, 24) : id(),
    url: clean(m && m.url, 600).trim(),
    caption: clean(m && m.caption, 160),
  })).filter((m) => m.url);
}
function sanitizeSite(o) {
  o = o || {};
  const keys = ['dhivehiName','heroEyebrow','heroTitle','heroSub','welcomeTitle','welcomeText','islandsEyebrow','islandsTitle','islandsP1','islandsP2','fact1n','fact1l','fact2n','fact2l','fact3n','fact3l','galleryTitle','galleryNote','aboutTitle','aboutP1','aboutP2','email'];
  const out = {};
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && req.query && req.query.get === 'status') {
    ...
    return res.status(200).json({
      ...
      storeVars: Object.keys(process.env).filter((k) => /KV|REDIS|UPSTASH|STORAGE/i.test(k)),
    });
  }
  ...
  if (req.method === 'GET') {
    ...
    const images = (await getJSON(K.images, [])) || [];
    res.setHeader('Cache-Control', 'no-store, max-age=0');
    return res.status(200).json({ site, apps, docs, images });
  }
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'save') {
      ...
      else if (kind === 'images') await setJSON(K.images, sanitizeImages(body.data));
